...
Scribe    & Recording   & Both      & Yes  & \texttt{architecture.md}, \texttt{log/*.md}
  & Reads all artifacts; writes docs only \\
...
\node[right=0.15cm of doc.north east, font=\tiny\sffamily, text=cGray,
      anchor=south west] {$\triangleright$ \texttt{arch.md} $\wedge$ \texttt{log/}};
...
\textsc{documented} &
  \texttt{architecture.md} in both run dir and target repo root;
  log entry with process record in \texttt{<repo>/log/};
  Scribe dispatched; dev artifacts excluded from release \\
...
\node[art, left=of arch] (logmd) {log/*.md};
...
artifact is committed to the target repository root (excluded from
release packages via \texttt{.Rbuildignore}, \texttt{.npmignore},
etc.).

\paragraph{Process-record log (\texttt{log/*.md}).}
...
entries accumulate in \texttt{<repo>/log/} as a traceable record of
...
\textbf{Profile} & \textbf{Validation Command} &
  \textbf{Doc System} & \textbf{Release Exclusion} \\
\midrule
