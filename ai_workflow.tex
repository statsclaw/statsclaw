...
Scribe    & Recording   & Both      & Yes  & \texttt{architecture.md}, \texttt{log-entry.md}
  & Reads all artifacts; writes to run dir (synced to brain repo) \\
...
\textsc{documented} &
  ...
  Scribe dispatched; github agent syncs to brain repo \\
...
artifact is written to the run directory and synced to the brain
repository (\texttt{[owner]/statsclaw-brain/<repo>/architecture.md})
by the GitHub agent---it is \emph{not} committed to the target repository,
keeping target repos clean.
...
entries are synced to the brain repository
(\texttt{[owner]/statsclaw-brain/<repo>/log/}) as a traceable record of
all code changes, enabling any future developer to understand not
just \emph{what} changed, but \emph{why} and \emph{how it was
verified}. The brain repo accumulates logs across all target
