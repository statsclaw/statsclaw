...
5. **Commit and push** to user's fork — apply the same `CommitTrailers` attribution as target repo commits. If enabled, the commit includes `Co-authored-by: StatsClaw <271365820+statsclaw@users.noreply.github.com>` so both the user and StatsClaw are credited.
