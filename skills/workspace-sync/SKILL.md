...
All workflow-generated logs, process records, architecture diagrams, and intermediate artifacts are stored in a dedicated **workspace repository** on GitHub. The user chooses the repo name (e.g., `[username]/workspace`, `[username]/my-logs`, etc.) and tells StatsClaw which repo to use. This keeps target repositories clean — only code and essential documentation live in the target repo.
...
│  └── git push workspace repo (architecture + log entry)     │
└─────────────────────────────────────────────────────────┘
```
...
cp "${RUN_DIR}/Architecture.md" "${WORKSPACE_DIR}/Architecture.md"
...
cp "${RUN_DIR}/log-entry.md" "${WORKSPACE_DIR}/log/${LOGFILE}"
...
| `Architecture.md` | **No** | Yes | Recorder writes to run dir; shipper syncs to workspace |
| `log/<date>-<slug>.md` | **No** | Yes | Recorder writes to run dir; shipper syncs to workspace |
