...
2. **`.env` file**: key `MINERU_API_TOKEN` — search current directory, then up to 5 parent directories
...
```
POST https://mineru.net/api/v4/file_parse
Content-Type: multipart/form-data

Fields:
  file: <PDF binary>
  model: "vlm"          # highest accuracy for academic papers
  enable_formula: true
  enable_table: true
```
...
```
GET https://mineru.net/api/v4/tasks/{task_id}
```

Poll every 5 seconds. Timeout after 10 minutes.

Response states: `pending`, `running`, `completed`, `failed`

### Step 3 — Download Results

```
GET https://mineru.net/api/v4/tasks/{task_id}/result
```

Returns a ZIP containing:
- `content_list.json` — typed elements with bounding boxes
...
The extraction script transforms MinerU's `content_list.json` into a StatsClaw-native format:
...
3. **Pseudocode density**: contains >= 3 of: `for`, `while`, `if`, `then`, `else`, `repeat`, `until`, `do`, `end`, `return` (case-insensitive)
4. **Numbered steps with control flow**: has >= 3 numbered lines (e.g., "1.", "2.") AND contains at least one control flow keyword
...
```
Usage: python extract_elements.py --input <content_list.json> --output <paper-elements.json> [--markdown <file.md>]

Options:
  --input       MinerU content_list.json path (required)
