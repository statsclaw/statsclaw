...
| "update docs" / "edit quarto book" / "fix README" / "write vignette" / docs-only | Docs only | Workflow 3 (theorist → scribe → auditor → skeptic) — NO builder |
...
- **MUST NOT fix bugs directly** — when auditor issues BLOCK, lead MUST respawn the responsible implementer (builder for code, scribe for docs) via `Agent` tool. Even if the fix appears trivial, lead MUST NOT apply it with Edit/Write/sed. Lead lacks validation context and may introduce new bugs.
