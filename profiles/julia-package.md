...
| `[deps]` section | List only direct dependencies; test-only dependencies go in `test/Project.toml` |
...
6. **Test-only dependencies**: Confirm test-only packages are in `test/Project.toml`, not the main `[deps]`.
...
- Place test-only dependencies in `test/Project.toml`, not in the main `[deps]`.
