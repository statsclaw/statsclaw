...
| `PAPER_PARSED` | `paper-elements.json` exists in run directory with ≥1 equation or table (workflow 14 only; skipped for other workflows) | Read file, verify non-empty extraction |
| `SPEC_READY` | `comprehension.md` and `spec.md` exist; `test-spec.md` also exists (except docs-only workflow 3, where it is not produced); `sim-spec.md` also exists for simulation workflows (11, 12, 14 with Monte Carlo) | Read file paths; for workflow 3, only `comprehension.md` + `spec.md` required; for simulation workflows, also verify `sim-spec.md` |
...
- **Workflow 14**: Paper-to-Package — PDF academic paper → planner comprehends paper (Paper Ingestion Mode) → generates specs → standard build/test pipeline. **Default: Claude reads the PDF directly** — no MinerU needed for most born-digital PDFs. MinerU is an optional enhancement (user can request it for scanned PDFs or equation-dense papers). When MinerU is used, leader runs `skills/paper-ingestion/` scripts to produce `paper-elements.json`, adding `PAPER_PARSED` state. When direct PDF mode is used, `PAPER_PARSED` is skipped. Planner MUST HOLD to confirm comprehension with the user. If the paper contains a Monte Carlo section, planner also produces `sim-spec.md` and the workflow includes simulator (parallel with builder). **If MinerU API fails** (timeout, network error, quota exceeded), leader MUST follow the Fallback Protocol in `skills/paper-ingestion/SKILL.md`. See `skills/paper-ingestion/SKILL.md`.
...
`CREDENTIALS_VERIFIED` → `NEW` → `PLANNED` → [`PAPER_PARSED`] → `SPEC_READY` → `PIPELINES_COMPLETE` → `DOCUMENTED` → [`KNOWLEDGE_EXTRACTED`] → `REVIEW_PASSED` → `READY_TO_SHIP` → `DONE`

Note: `PAPER_PARSED` is optional — only entered in workflow 14 (Paper-to-Package) after MinerU API parsing completes and `paper-elements.json` is written. Skipped for all other workflows.
